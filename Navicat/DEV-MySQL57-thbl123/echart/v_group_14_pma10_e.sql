...
FROM
	...
	(
SELECT
	`dim_date_14_e`.`date_v` AS `date_v` 
FROM
	`echart`.`dim_date_14_e` 
WHERE
	( ( `dim_date_14_e`.`date_v` >= str_to_date( '2015-08-03', '%Y-%m-%d' ) ) AND ( `dim_date_14_e`.`date_v` <= str_to_date( '2019-05-01', '%Y-%m-%d' ) ) ) 
ORDER BY
	`dim_date_14_e`.`date_v` 
	) `dim_table`
	LEFT JOIN (
SELECT
	`v_group_14_e`.`tgroup` AS `tgroup`,
