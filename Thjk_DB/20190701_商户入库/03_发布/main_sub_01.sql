...
-- -------------------------------
drop table if exists `dw_2g`.`tmp_dwd_merchant_info`;

CREATE TABLE `dw_2g`.`tmp_dwd_merchant_info` (
...
insert into table `dw_2g`.`tmp_dwd_merchant_info`
    partition (act_start)
select
    `dt_type`,
    `mcht_cd`,
...
from `dw_2g`.`dwd_merchant_info`
LEFT SEMI JOIN (
    SELECT mcht_cd as mcht_cd2 from ods_ftp.merchant_update where p_date=${THE_DATE}
) aaa
on dwd_merchant_info.mcht_cd = aaa.mcht_cd2
;
