...
SELECT
    ...
    ,dwd_merchant_info.bran_cd              as `机构代码`       -- 机构代码
    ,concat(
    ...
    ,dwd_merchant_info.reg_prov_cd          as `省份码`        -- 省份码
    ,dwd_merchant_info.reg_city_cd          as `城市码`        -- 城市码
    ,dwd_merchant_info.reg_country_cd       as `区县码`        -- 区县码
    ,dwd_merchant_info.legal_person_name    as `法人姓名`       -- 法人姓名
--    ,upper(regexp_replace(dwd_merchant_info.legal_person_id_no,'\\s','')) as `身份证`  -- 身份证号码去空格大写
    ,dwd_merchant_info.mcht_name            as `商户名称`               -- 商户名称
    ,dwd_merchant_info.contact_phone        as `联系电话`               -- 联系电话
...
LEFT JOIN dim.dwd_merchant_info         -- 商户基础信息表
on dm_qsd_merchant.merchant_ap=dwd_merchant_info.mcht_cd           -- 商户号相等
...
on dwd_merchant_info.bran_cd = aaa.bran_cd  -- 机构编号相等
left join
(                               -- 资产保理注册用户手机号码
    SELECT
 ...
 on upper(regexp_replace(dwd_merchant_info.legal_person_id_no,'\\s','')) = fff.legal_id_no  -- 用身份证号码关联
order by
    `机构名称` desc
    ,`地区名称`
    ,`法人姓名`
