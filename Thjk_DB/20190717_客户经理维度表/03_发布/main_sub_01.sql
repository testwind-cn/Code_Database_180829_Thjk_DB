...
CREATE TABLE IF NOT EXISTS dim.dim_manager_code (
...
DROP TABLE IF EXISTS dim.tmp_dim_manager_code;
CREATE TABLE  IF NOT EXISTS dim.tmp_dim_manager_code (
    `platform_code` string COMMENT '平台代码',
    `manager_code`  string COMMENT '客户经理代码',
    `cert_no`  string COMMENT '身份证',
    `name`  string COMMENT '姓名',
...
CREATE TEMPORARY TABLE  dim.tmp_eee (
    `platform_code` string COMMENT '平台代码',
    `manager_code`  string COMMENT '客户经理代码',
    `cert_no`  string COMMENT '身份证',
    `name`  string COMMENT '姓名',
...
CREATE TEMPORARY TABLE  dim.tmp_ggg (
...
(
    SELECT
        lower(regexp_replace(coalesce(dim_manager_code.platform_code,'null'),'\\s','')) platform_code
        , lower(regexp_replace(coalesce(dim_manager_code.manager_code,'null'),'\\s','')) manager_code
        , upper(regexp_replace(coalesce(dim_manager_code.cert_no,'NULL'),'\\s','')) cert_no
        , lower(regexp_replace(coalesce(dim_manager_code.name,'null'),'\\s','')) name
        , lower(regexp_replace(coalesce(dim_manager_code.phone,'null'),'\\s','')) phone
        , lower(regexp_replace(coalesce(dim_manager_code.email,'null'),'\\s','')) email
        , coalesce(dim_manager_code.manager_id,0) manager_id
    from dim.dim_manager_code
)
insert into table dim.tmp_eee
...
insert into table dim.tmp_dim_manager_code
select
    ggg.platform_code
    , ggg.manager_code
...
select
    ...
    , eee.manager_id
from dim.tmp_eee eee
left join dim.tmp_ggg ggg
on
